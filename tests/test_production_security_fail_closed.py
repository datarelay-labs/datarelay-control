...
def test_lab_platform_compose_defaults_to_loopback_binds() -> None:
    text = (ROOT / "docker-compose.platform.yml").read_text(encoding="utf-8")
    assert "${GDC_API_BIND:-127.0.0.1}:${GDC_API_HOST_PORT:-8000}:8000" in text
...
def test_https_compose_has_no_docker_socket() -> None:
    text = (ROOT / "deploy/docker-compose.https.yml").read_text(encoding="utf-8")
