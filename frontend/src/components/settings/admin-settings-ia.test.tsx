...
describe('AdminSettingsPage IA modernization', () => {
  ...
  it('shows purpose, section hierarchy, and hub return link', async () => {
    ...
    expect(within(lifecycle).queryByRole('heading', { name: 'Maintenance Center' })).not.toBeInTheDocument()
    expect(within(lifecycle).queryByRole('heading', { name: 'Health monitoring' })).not.toBeInTheDocument()
    expect(within(lifecycle).queryByRole('heading', { name: 'Audit log' })).not.toBeInTheDocument()
    expect(within(operations).getByRole('heading', { name: 'Support bundle' })).toBeInTheDocument()
    expect(within(operations).getByRole('heading', { name: 'Maintenance Center' })).toBeInTheDocument()
    expect(within(operations).getByRole('heading', { name: 'Health monitoring' })).toBeInTheDocument()
    const maintenance = within(operations).getByRole('heading', { name: 'Maintenance Center' })
    const support = within(operations).getByRole('heading', { name: 'Support bundle' })
    expect(maintenance.compareDocumentPosition(support) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
    expect(screen.getByTestId('admin-open-system-information')).toBeInTheDocument()
    expect(screen.getByText(/Maintenance readiness, support evidence/i)).toBeInTheDocument()
